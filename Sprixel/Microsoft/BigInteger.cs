...
namespace Sprixel {
    ...
    public sealed class BigInteger : IFormattable, IComparable, IConvertible, IEquatable<BigInteger> {
        ...
        public static BigInteger Create(string v) {
            var length = v.Length;
            bool negative = false;
            if (length == 0)
                throw new Exception("blank input to new BigInteger");
            if (v[0] == '-') {
                negative = true;
                v = v.Substring(1);
                --length;
            ...
            int mag = 0;
            var value = BigInteger.Create(0);
            var fifteen = BigInteger.Create(UInt64.Parse("1000000000000000"));
            while (length >= 15) {
                value *= fifteen;
                value += UInt64.Parse(v.Substring(15 * mag++, 15));
                length -= 15;
            }
            if (length > 0) {
                value *= BigInteger.Create(Math.Pow(10, length));
                value += UInt64.Parse(v.Substring(mag * 15, length));
            }
